...
- A source column labelled "Vat %", "VAT %", or any similar percentage label supplies a VAT rate, not a VAT amount; place its value in line.vatRate, not line.vatAmount.
- Do not assume a source column named "Amount", "Price", or any unlabelled money column is always net. Read the column header and reconcile against invoice-level totals to determine what each column represents.
- To determine whether source line amounts are net or VAT-inclusive: check which invoice-level total their sum matches. If the sum of source line amounts equals the invoice Net, they are net amounts — populate line.netAmount. If the sum equals the invoice Total or Gross, they are VAT-inclusive amounts — populate line.grossAmount instead.
...
- When source table column semantics are ambiguous, use invoice-level netAmount, vatAmount, and grossAmount as reconciliation anchors to resolve what each column represents.
