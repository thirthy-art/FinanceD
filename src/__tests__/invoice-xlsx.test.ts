...
describe("invoice XLSX export", () => {
  it("contains both worksheets and one row for each of six invoice lines", async () => {
    const invoice = {
      id: 7, vendorName: "ACME", vendorTaxId: "CY123", invoiceNumber: "INV-7",
      invoiceDate: "2026-08-15", dueDate: null, currency: "EUR", currencyType: "fiat" as const,
    ...
    const lines: InvoiceLineExportRow[] = Array.from({ length: 6 }, (_, index) => ({
      ...
      invoiceDate: "2026-08-15", invoiceStatus: "approved", currency: "EUR", currencyType: "fiat",
    ...
    expect(workbook.getWorksheet("Invoices")?.getCell("H2").numFmt).toBe("0.00");
