...
import { AccountDeleteAction, OpeningBalanceActions, TransactionActions } from "./PaymentAccountMutationControls";
...
export default function PaymentAccountDetail({ detail, messages }: { detail: PaymentAccountDetailView; messages: Messages["paymentAccounts"] }) {
  ...
  return <div className="payment-account-detail-page">
    ...
    <Card className="mb-5"><CardHeader><CardTitle>{messages.accountHistory}</CardTitle></CardHeader><CardContent><div className="grid gap-5">
      {detail.openings.length > 0 && <OpeningBalanceActions account={detail.account} openings={detail.openings.map((opening) => ({ ...opening, openingAvailableBalance: amount(opening.openingAvailableBalance), openingReserveBalance: amount(opening.openingReserveBalance) }))} messages={messages}/>}
      {detail.events.length === 0 ? (detail.openings.length === 0 ? <EmptyState>{messages.noTransactions}</EmptyState> : null) : <TransactionActions events={detail.events.map((event) => ({ ...event, balanceAmount: amount(event.balanceAmount) }))} messages={messages}/>}
