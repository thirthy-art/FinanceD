...
export type PaymentAccountDetailView = {
  ...
  events: Array<{ id: number; eventDate: string; eventType: PaymentEventType; balanceDirection: BalanceDirection; balanceAmount: string; balanceAssetCode: string; providerEventId: string | null; reference: string | null }>;
...
export default function PaymentAccountDetail({ detail, messages }: { detail: PaymentAccountDetailView; messages: Messages["paymentAccounts"] }) {
  ...
  return <div className="payment-account-detail-page">
    ...
    <Card className="mb-5"><CardHeader><CardTitle>{messages.transactions}</CardTitle></CardHeader><CardContent>{detail.events.length === 0 ? <EmptyState>{messages.noTransactions}</EmptyState> : <Table headers={[messages.eventDate, messages.eventType, messages.amount, messages.providerEventId]} rows={detail.events.map((event) => [event.eventDate, messages.eventTypeLabels[event.eventType], `${messages.directionLabels[event.balanceDirection]} ${amount(event.balanceAmount)} ${event.balanceAssetCode}`, event.providerEventId ?? "—"])}/>}</CardContent></Card>
