...
import { getPaymentAccountDetail } from "@/src/lib/payment-ledger";
...
export default async function PaymentAccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await getPaymentAccountDetail(company.id, paymentAccountId); if (!result) notFound();
  ...
  return <PaymentAccountDetail detail={detail} messages={getMessages(locale).paymentAccounts}/>;
}
